...
def _get_temperature_data_eeweather(usaf_id, start,
    ...
    if normalized:
        if use_cz2010:
            tempC, warnings = eeweather.load_cz2010_hourly_temp_data(usaf_id, start, end)
        else:
            tempC, warnings = eeweather.load_tmy3_hourly_temp_data(usaf_id, start, end)
    else:
...
class WeatherSource(object):
    ...
    def indexed_temperatures(self, index, unit, allow_mixed_frequency=False):
        ...
        self.tempC = _get_temperature_data_eeweather(self.usaf_id, start, end, self.normalized, self.use_cz2010)
